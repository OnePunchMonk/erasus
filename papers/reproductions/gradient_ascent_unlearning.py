"""
Reproduction: Gradient Ascent for Machine Unlearning (Thudi et al., 2022).

Reproduces the core experiment: training a classifier on CIFAR-10,
then unlearning a subset using gradient ascent, and verifying with
MIA metrics.
...
def main():
    print("=" * 60)
    print("  Paper Reproduction: Gradient Ascent Unlearning")
    print("  Thudi et al. (2022)")
    print("=" * 60)
    ...
    print(f"  Model: SimpleResNet ({sum(p.numel() for p in model.parameters()):,} params)")
    ...
    retain_data = TensorDataset(
        torch.randn(400, 3, 32, 32), torch.randint(0, 10, (400,))
    )
    ...
    # ---- Pre-training (simulate) ----
    print("\n  Phase 1: Pre-training (simulated)...")
    ...
    # ---- Unlearning ----
    print("\n  Phase 3: Gradient Ascent Unlearning...")
    unlearner = ErasusUnlearner(
        model=model,
        strategy="gradient_ascent",
        selector=None,  # Full forget set
        device=device,
    ...
    print("\n  Expected: forget accuracy ↓, retain accuracy stable, MIA AUC → 0.5")
